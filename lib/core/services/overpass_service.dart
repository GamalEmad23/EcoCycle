      ...
      try {
        final response = await http
            .post(
              ...
              headers: {
                'User-Agent':
                    'com.ecocycle.app', // Required by Overpass API policy
        ...
        if (response.statusCode == 200) {
          // Overpass responses contain multilingual OSM tags. Decode the raw
          // bytes explicitly as UTF-8 so Arabic names are never treated as
          // Latin-1 when the server omits a charset header.
          ...
          print(
            'Overpass API Error (Try ${currentTry + 1}): ${response.statusCode} - ${response.body}',
          );
          if (currentTry == maxRetries - 1) {
            throw Exception(
              'Failed to load centers. Server responded with: ${response.statusCode}',
            );
          }
        }
      } catch (e) {
        print('Overpass API Request Exception (Try ${currentTry + 1}): $e');
        if (currentTry == maxRetries - 1) {
