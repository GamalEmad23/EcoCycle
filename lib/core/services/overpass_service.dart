    ...
    while (currentTry < maxRetries) {
      try {
        final response = await http.post(
          ...
          headers: {
            'User-Agent': 'com.ecocycle.app', // Required by Overpass API policy
        ...
        if (response.statusCode == 200) {
          ...
          print('Overpass API Error (Try ${currentTry + 1}): ${response.statusCode} - ${response.body}');
          if (currentTry == maxRetries - 1) {
            throw Exception('Failed to load centers. Server responded with: ${response.statusCode}');
          }
        }
      } catch (e) {
