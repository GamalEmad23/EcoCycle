            ...
            child: BlocBuilder<StatisticsCubit, StatisticsState>(
              builder: (context, state) {
                ...
                if (_selectedRange == "statistics.last_week") {
                  ...
                  chartSpots = dailyOps.asMap().entries.map((e) => FlSpot(e.key.toDouble(), e.value)).toList();
                  ...
                  chartLabels = List.generate(8, (i) {
                    ...
                    return DateFormat('ha', context.locale.languageCode).format(time);
                  });
                  ...
                  chartSpots = hourlyOps.asMap().entries.map((e) => FlSpot(e.key.toDouble(), e.value)).toList();
                  ...
                  for (var activity in recentActivities) {
                    if (activity.createdAt != null) {
                      final diffDays = now.difference(activity.createdAt!).inDays;
                  ...
                  chartSpots = weeklyOps.asMap().entries.map((e) => FlSpot(e.key.toDouble(), e.value)).toList();
                  ...
                  for (var activity in recentActivities) {
                    if (activity.createdAt != null) {
                      final diffMonths = (now.year - activity.createdAt!.year) * 12 + now.month - activity.createdAt!.month;
                      if (diffMonths >= 0 && diffMonths < 6) {
                        monthlyOps[5 - diffMonths] += 1;
                      }
                    }
                  }
                  chartSpots = monthlyOps.asMap().entries.map((e) => FlSpot(e.key.toDouble(), e.value)).toList();
                }

                
                // Ensure chart looks nice if all values are zero
                if (chartSpots.every((spot) => spot.y == 0)) {
                   chartSpots = chartSpots.asMap().entries.map((e) => FlSpot(e.key.toDouble(), 0.1)).toList();
                }
                        ...
                        children: [
                          Expanded(
                            child: SmallCard(
                              ...
                              subtitle: "$co2Trend ${"statistics.this_month".tr()}",
                            ),
                          ),
                          ...
                          Expanded(
                            child: SmallCard(
                              ...
                              subtitle: "$operationsTrend ${"statistics.this_month".tr()}",
                            ),
                          ),
                        ],
                      ),
                      ...
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: [
                              ...
                              children: [
                                const CustomeText(
                                 ...
                                 PopupMenuButton<String>(
                                  ...
                                  itemBuilder: (context) => [
                                    PopupMenuItem(value: "statistics.today", child: Text("statistics.today".tr())),
                                    PopupMenuItem(value: "statistics.last_week", child: Text("statistics.last_week".tr())),
                                    PopupMenuItem(value: "statistics.last_month", child: Text("statistics.last_month".tr())),
                                    PopupMenuItem(value: "statistics.Last_months", child: Text("statistics.Last_months".tr())),
                                  ],
                                  child: Container(
                                    padding: const EdgeInsets.symmetric(
                                      horizontal: 12,
                            ...
                            Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              children: chartLabels.map((label) => MonthText(label)).toList(),
                            ),
                          ],
                        ),
                      ),
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: activities.map((activity) {
        ...
        } else if (title.contains("معدن") || title.toLowerCase().contains("metal")) {
           translatedTitle = "statistics.metal".tr();
           imagePath = "assets/images/metal.png";
           iconBg = const Color(0xFFFFE9D6);
           iconColor = Colors.orange;
        } else if (title.contains("بلاستيك") || title.toLowerCase().contains("plastic")) {
           translatedTitle = "statistics.plastic".tr();
           imagePath = "assets/images/plastic.png";
           iconBg = const Color(0xFFDDF5F0);
           iconColor = Colors.teal;
        } else if (title.contains("إلكترونيات") || title.toLowerCase().contains("electronics")) {
           translatedTitle = "statistics.electronics".tr();
           imagePath = "assets/images/description.png"; // Fallback to description icon
        ...
        return Padding(
          ...
          child: ActivityItem(
            ...
            subtitle: subtitle,
            weight: weight,
            points: "+${(activity.weight * 5).toInt()} ${"statistics.points".tr()}",
            imagePath: imagePath,
            iconBg: iconBg,
            iconColor: iconColor,
          ),
